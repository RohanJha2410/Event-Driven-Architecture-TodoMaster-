...
export async function GET(req: NextRequest) {
  ...
  if (!email) {
    return NextResponse.json(
      { user: null, totalPages: 0, currentPage: 1 },
      { status: 200 }
    );
  }
  ...
  try {
    ...
    const totalPages = Math.max(1, Math.ceil(totalTodos / ITEMS_PER_PAGE));
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (typeof todoId === "string" && typeof todoCompleted === "boolean") {
    ...
    if (typeof isSubscribed === "boolean" && typeof email === "string") {
